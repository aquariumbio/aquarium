...
module Krill
  ...
  module OperationList
    ...
    def get(key, opts)
      @table.add_column(opts[:heading] || key.to_s, running.each_with_index.collect do |op, ii|
        { type: opts[:type] || 'number', operation_id: op.id, key: key, default: opts[:default] || 0, row: ii }
      end)
      self
    end
