  ...
  def main
    ...
    # Determine replacements of e coli comp cell batch
        operations.running.each do |op|
            ...
            if op.input(CELLS).collection.empty? or (debug and DEBUG_WITH_REPLACEMENT)
                old_batch = op.input(CELLS).collection
                
                # Find replacement batches
                all_batches = Collection.where(object_type_id: op.input(CELLS).object_type.id).keep_if { |b| !b.empty? }
