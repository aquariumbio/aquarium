...
RSpec.describe ShowResponse do
	context "For a simple example ShowResponse " do
		...
		it "returns a ruby hash representing the data with responses" do
	    	expect(resp.responses()).to eq({measured_concentration: 53.2, tblrespnskey: [2, 1]})
	    end
		...
		it "returns a sorted array corresponding to rows of the table with get_response when the given key is the name of an table response" do
		...
		it "raises an error if you misuse the interface for get_table_response (which requires exactly one optional argument)" do
			...
			expect{resp.get_table_response(:tblrespnskey)}.to raise_error(TableCellUndefined)
			expect{resp.get_table_response(:tblrespnskey, op: 3075, row: 0)}.to raise_error(TableCellUndefined)
		end
    end
    
	it "works with large and complex response hashes" do
		bigresp = ShowResponse.new({
			table_inputs: [ {key: "tblrespnskey", opid: 3075, row: 0, value: 2, type: "number"}, {key: "tblrespnskey", opid: 3076, row: 1, value: 1, type: "number"},  {key: "tblrespnskey", opid: 3077, row: 2, value: 3, type: "number"},  {key: "tblrespnskey", opid: 3078, row: 3, value: 4, type: "number"},  {key: "tblrespnskey", opid: 3080, row: 4, value: 5, type: "number"},  {key: "tblrespnskey", opid: 3079, row: 5, value: 6, type: "number"}, {key: "tblrespnskey2", opid: -4, row: 3, value: "four", type: "text"},  {key: "tblrespnskey2", opid: -5, row: 4, value: "five", type: "text"},  {key: "tblrespnskey2", opid: -6, row: 5, value: "six", type: "text"}, {key: "tblrespnskey2", opid: -1, row: 0, value: "one", type: "text"}, {key: "tblrespnskey2", opid: -2, row: 1, value: "two", type: "text"},  {key: "tblrespnskey2", opid: -3, row: 2, value: "three", type: "text"}, ], 
			response1: "SUPERLONGSTRINGSUPERLONGSTRINGSUPERLONGSTRINGSUPERLONGSTRINGSUPERLONGSTRING",
		...
		expect(bigresp.get_response(:response2)).to eq(1412312312312312312312412312312312)
		expect(bigresp.get_response(:response3)).to eq("one more datum")
		expect(bigresp.get_table_response(:tblrespnskey, op: Operation.find(3079))).to eq(6)
		...
		expect{bigresp.get_table_response(:tblrespnskey, op: 5000)}.to raise_error
		expect{bigresp.get_table_response(:tblrespnskey, row: 100)}.to raise_error(TableCellUndefined)
	end	
end