...
module ApiCreate
  ...
  def create_job(_args)
    error 'Create job not yet implemented'
  end
  ...
  def create_workflow_thread(args)
    ...
    wf_form[:inputs].each do |wf_form_input|
      next unless wf_form_input[:alternatives]
      next unless wf_form_input[:alternatives][0][:sample_type]
      wf_form_sample_type_id = wf_form_input[:alternatives][0][:sample_type].split(':')[0]
      threads.each do |ip_thread|
    ...
    thread_ids = threads.collect do |thread|
