  ...
  def instantiate(output, desired_value)
    ...
    output.allowable_field_types.each do |output_aft|
      desired_value.field_type.allowable_field_types.each do |input_aft|
        aft = output_aft if output_aft.equals input_aft
      end
    end

    op.set_output(output.name, desired_value.child_sample, aft)

    raise "Could not set output of operation: #{op.errors.full_messages.join(', ')}" unless op.errors.empty?
    ...
    inputs.each do |i|
      ...
      if i.routing == output.routing
        ...
        raise "Could not set input of operation: #{op.errors.full_messages.join(', ')}" unless op.errors.empty?
