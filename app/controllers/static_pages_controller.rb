...
class StaticPagesController < ApplicationController
  ...
  def jobchart
    ...
    jobs.all.each do |j|
      ...
      start_entry = j.logs.select { |l| l.entry_type == 'START' }
      stop_entry = j.logs.select { |l| l.entry_type == 'STOP' }
      ...
      j[:start] = if !start_entry.empty?
                    start_entry.first.created_at.to_i
                  else
                    '0'
                  end

      j[:stop] = if !stop_entry.empty?
                   stop_entry.first.created_at.to_i
                 else
                   '0'
                 end
      ...
      result[login] = [] unless result.key? login
