...
class JsonController::JsonQueryResult
  ...
  def self.apply_method(model:, method:, arguments:, id:, options:)
    ...
    result = result.limit(options[:limit]) if options[:limit] && options[:limit].to_i > 0
    result = result.offset(options[:offset]) if options[:offset] && options[:offset].to_i > 0
