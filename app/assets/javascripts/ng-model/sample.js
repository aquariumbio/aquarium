...
Sample.prototype.get_inventory = function(promise) {
  ...
  this.http.get("/browser/items/" + this.id + ".json").then(function(response) {
    ...
    sample.http
      .get("/browser/collections/" + sample.id + ".json")
      .then(function(response) {
        sample.collections = aq.collect(response.data.collections, function(
          raw
        ) {
  ...
  aq.each(this.field_values, function(fv) {
    ...
    if (t == "number") {
      ...
      if (fv.child_sample) {
        ...
        fv.child_sample_name =
          "" + fv.child_sample.id + ": " + fv.child_sample.name;
      }
...
Sample.prototype.update = function(promise) {
  this.http
    .put("/samples/" + this.id + ".json", { sample: this })
    .then(function(response) {
...
Sample.prototype.create = function(promise) {
  this.http.post("/samples.json", { sample: this }).then(function(response) {
    promise(response.data);
  });
  return this;
};
...
Sample.prototype.lookup = function(name, sample_names, types, warnings) {
  for (var st_name in sample_names) {
    for (var i = 0; i < sample_names[st_name].length; i++) {
      ...
      if (
        types.indexOf(st_name) >= 0 &&
        (identifier.split(": ")[0] == name || identifier.split(": ")[1] == name)
      ) {
        return identifier;
      }
    }
  ...
  warnings.push(
    "Sample " +
      name +
      " was not found in among samples allowed for the given sample type and field."
  );
  return "UNKNOWN SUBSAMPLE " + name;
};

Sample.prototype.assign_fv_aux = function(
  t,
  fv,
  value,
  sample_names,
  ...
  if (t.ftype == "number") {
    var nchoices = aq.collect(choices, function(s) {
      return parseFloat(s);
    });
    if (!choices || nchoices.indexOf(parseInt(value)) >= 0) {
      ...
      warnings.push(
        "Value for field " + t.name + " should be one of " + t.choices + "."
      );
    }
  } else if (t.ftype == "sample") {
    if (!choices || choices.indexOf(value) >= 0) {
      fv.child_sample_name = this.lookup(
        value,
        sample_names,
        fv.allowable_child_types,
        warnings
      );
    } else {
      warnings.push(
...
Sample.prototype.assign_field_value = function(
  name,
  value,
  sample_names,
  warnings
