  ...
  w.controller("browserCtrl", [
    ...
    function($scope, $http, $attrs, aqCookieManager, $sce, $window) {
      ...
      $scope.item_search = function() {
        AQ.Item.find($scope.views.search.item_id)
          .then(item => {
            $scope.views.search.item = item;
            $scope.views.search.item.modal = true;
            $scope.views.search.item.new_location =
              $scope.views.search.item.location;
            cookie();
            AQ.update();
          })
          .catch(() =>
            alert("Could not find item with id " + $scope.views.search.item_id)
          );
      };
