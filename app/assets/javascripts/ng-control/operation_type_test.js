(function() {
  ...
  w.controller("operationTypeTestCtrl", [
    "$scope",
    "$http",
    "$attrs",
    "$cookies",
    ...
    function($scope, $http, $attrs, $cookies, $sce) {
      ...
      $scope.test = function(operation_type) {
        if (
          ...
          operation_type.test_operations.length > 0
        ) {
          operation_type.test_results = null;
          operation_type.test_error = null;
          operation_type.running_test = true;
          $http
            .post("/operation_types/test", operation_type.remove_predecessors())
            .then(
              function successCallback(response) {
                operation_type.running_test = false;
                aq.each(operation_type.field_types, ft =>
                  ft.recompute_getter("predecessors")
                );
                operation_type.test_results = response.data;
                operation_type.test_results.job.backtrace = JSON.parse(
                  operation_type.test_results.job.state
                );
              },
              function failureCallback(response) {
                operation_type.running_test = false;
      ...
      $scope.backtrace = function(step) {
        ...
        return aq.collect(relevant_messages, function(msg) {
          return msg.replace(/\(eval\):/g, "Line ");
        });
      };
