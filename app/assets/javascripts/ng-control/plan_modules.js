...
AQ.Plan.record_methods.wire_in_module = function(current, wire) {
  var p1 = wire.from_module
      ? wire.from_module.parent_id
      : wire.from_op.parent_id,
...
AQ.Plan.record_methods.add_module_wires_from_module_wires = function(
  new_module
) {
  var plan = this,
    current = plan.current_module;
...
AQ.Plan.record_methods.delete_old_module_wires = function(new_module) {
  ...
  plan.current_module.wires = aq.where(
    plan.current_module.wires,
    w => !plan.uses_module(w, new_module)
  );
};
