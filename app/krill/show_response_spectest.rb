...
Rspec.describe ShowResponse
	it "translates the response hash from a show block into an object with a simpler interface" do
		...
		it "is backwards compatible with the original hash" do
			expect(resp).to eq({table_inputs: [], timestamp: 123456789, measured_concentration: 53.2 })
		end
		...
		it "returns a ruby hash representing the data with responses" do
	    	expect(resp.responses()).to eq({measured_concentration: 53.2})
	    end
